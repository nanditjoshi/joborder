<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\AdminJoborder[]|\Cake\Collection\CollectionInterface $adminJoborder
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('New Admin Joborder'), ['action' => 'add']) ?></li>
    </ul>
</nav>
<div class="adminJoborder index large-9 medium-8 columns content">
    <h3><?= __('Admin Joborder') ?></h3>
    <table cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                <th scope="col"><?= $this->Paginator->sort('id') ?></th>
                <th scope="col"><?= $this->Paginator->sort('job-type') ?></th>
                <th scope="col"><?= $this->Paginator->sort('job-status') ?></th>
                <th scope="col"><?= $this->Paginator->sort('job-title') ?></th>
                <th scope="col"><?= $this->Paginator->sort('start-date') ?></th>
                <th scope="col"><?= $this->Paginator->sort('preferred-country') ?></th>
                <th scope="col"><?= $this->Paginator->sort('employer') ?></th>
                <th scope="col"><?= $this->Paginator->sort('agencies') ?></th>
                <th scope="col"><?= $this->Paginator->sort('candidates') ?></th>
                <th scope="col"><?= $this->Paginator->sort('documents') ?></th>
                <th scope="col"><?= $this->Paginator->sort('created') ?></th>
                <th scope="col"><?= $this->Paginator->sort('modified') ?></th>
                <th scope="col" class="actions"><?= __('Actions') ?></th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($adminJoborder as $adminJoborder): ?>
            <tr>
                <td><?= $this->Number->format($adminJoborder->id) ?></td>
                <td><?= h($adminJoborder->job-type) ?></td>
                <td><?= h($adminJoborder->job-status) ?></td>
                <td><?= h($adminJoborder->job-title) ?></td>
                <td><?= h($adminJoborder->start-date) ?></td>
                <td><?= h($adminJoborder->preferred-country) ?></td>
                <td><?= h($adminJoborder->employer) ?></td>
                <td><?= h($adminJoborder->agencies) ?></td>
                <td><?= h($adminJoborder->candidates) ?></td>
                <td><?= h($adminJoborder->documents) ?></td>
                <td><?= h($adminJoborder->created) ?></td>
                <td><?= h($adminJoborder->modified) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $adminJoborder->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $adminJoborder->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $adminJoborder->id], ['confirm' => __('Are you sure you want to delete # {0}?', $adminJoborder->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
    <div class="paginator">
        <ul class="pagination">
            <?= $this->Paginator->first('<< ' . __('first')) ?>
            <?= $this->Paginator->prev('< ' . __('previous')) ?>
            <?= $this->Paginator->numbers() ?>
            <?= $this->Paginator->next(__('next') . ' >') ?>
            <?= $this->Paginator->last(__('last') . ' >>') ?>
        </ul>
        <p><?= $this->Paginator->counter(['format' => __('Page {{page}} of {{pages}}, showing {{current}} record(s) out of {{count}} total')]) ?></p>
    </div>
</div>
